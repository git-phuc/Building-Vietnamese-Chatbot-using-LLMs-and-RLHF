...
#### Notebook A — chuẩn bị data (chạy đúng 1 lần, không cần GPU) — file: `notebooks/CPT-Step-A-Prepare-Qwen3-1.7B.ipynb`
...
#### Notebook B — train (chạy lại mỗi session, tự resume) — file: `notebooks/CPT-Step-B-Train-Qwen3-1.7B.ipynb`
...
> **Notebook chạy được thật: `notebooks/sft_train.ipynb`** — cùng cơ chế checkpoint/resume với `CPT-Step-B-Train-Qwen3-1.7B.ipynb` (Run All mỗi session, tự kéo checkpoint từ Hub, push nguyên folder checkpoint mỗi `save_steps`, tự dừng theo ngân sách giờ). Chỉ chạy sau khi Bước 1 đã push bản merged.
