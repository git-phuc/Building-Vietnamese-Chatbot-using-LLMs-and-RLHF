...
|---|---|---|---|---|
| 1 | CPT tiếng Việt (QLoRA r=64) | §6.1 | `<user>/Qwen3-1.7B-vi-cpt` | 🟡 **đang chuẩn bị train** — kế hoạch 2 notebook đã chốt ở §6.1 |
...
1. Tạo Kaggle secret `HF_TOKEN` (Add-ons → Secrets).
2. Chạy **Notebook A** (§6.1) đúng 1 lần → data đã pack nằm ở `<user>/vi-cpt-corpus-2048`.
3. Chạy **Notebook B** (§6.1) mỗi session — tự resume, tự dừng trước khi hết giờ; lặp đến khi đạt `max_steps=6000` thì notebook tự push bản merged `<user>/Qwen3-1.7B-vi-cpt`.
