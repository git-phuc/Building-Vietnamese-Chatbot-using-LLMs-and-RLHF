...
|---|---|---|---|---|
| 1 | CPT tiếng Việt (QLoRA r=64) | §6.1 | `<user>/Qwen3-1.7B-vi-cpt` | 🟡 **Notebook A ✅ xong (2026-07-18)** — data tại `Phuc-HugigFace/vi-cpt-corpus-2048`. **Notebook B: TRAIN LẠI TỪ ĐẦU (v2, 2026-07-22)** — bản v1 (`Qwen3-1.7B-vi-cpt-ckpt`, dừng ở step 2228, ~65 GPU-h) bị loại: eval_vi loss của adapter tệ hơn base gốc ~0.8 nats, không đổi qua 4 checkpoint, nghi do `use_rslora=True` (scale gấp 8 LoRA chuẩn). v2 tắt `use_rslora`, giữ nguyên r=64/lr=1.5e-5, checkpoint tại `Qwen3-1.7B-vi-cpt-ckpt-v2` — xem `docs/compute-log.md`. Session sau: tick lại 2 secret, Run All — tự bắt đầu lại từ 0 |
